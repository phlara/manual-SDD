...
This repo's SDD planning workflow lives in `ai-specs/.commands/`:
- `plan-backend-ticket.md` — plan a backend ticket (no implementation)
- `plan-frontend-ticket.md` — plan a frontend ticket (no implementation)
- `develop-backend.md` / `develop-frontend.md` — implement an approved plan

The `backend-developer` and `frontend-developer` agents in `ai-specs/.agents/` are the ones invoked by the planning commands.
